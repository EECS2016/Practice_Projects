/*
 *EasyBuy项目的JSP的function库 
 *
 *有无JQuery框架的JS函数库各提供一份
*/
//后端JSP的function库
 ...
 //后端主页：BE_Main.jsp
  //自动更新的日期函数
   function BEDate(){
    ...
    window.document.getElementById("BEDate").innerHTML="，您好！现在是"+year+"年"+(month+1)+"月"+date+"日，"
                                                       +hours+"时"+minutes+"分"+seconds+"秒"+"，欢迎回到管理后台。";
   }   
   $(window.document).ready(function (){window.setInterval(BEDate,1000);});
 ...
 //前端JSP的function库
  ...
  $(function() {
   ...
   function checkItem(dom) {
	...
	case "birthday": if ((value != "") && !/\d{4}[-]\d{1,2}[-]\d{1,2}/.test(value)) {
				      errorMessage = "出生日期格式只能是yyyy-mm-dd";
			         }else{
				       isError = false;
			          }break;
	case "identityCode": if ((value != "") && !/((\d{17}\w{1})|(\d{15}))/.test(value)) {
				          errorMessage = "身份证号码格式不正确";
			             }else{
				           isError = false;
			              }break;
	case "email": if((value != "")&& !/^[a-z0-9]+\@([a-z0-9]+\.)+[a-z0-9]{2,4}$/i.test(value)) {
				   errorMessage = "邮件地址格式不正确";
			      }else{
			   	    isError = false;
			       }break;
	case "mobile": if((value == "") || !/\d{11}/.test(value)){
				    errorMessage = "手机号码格式不正确";
			       }else{
				     isError = false;
			        }break;
	case "productName": if(value == ""){
				         errorMessage = "名称不能为空";
			            }else{
				          isError = false;
			             }break;
	case "productPrice": if((value == "")|| !(/^\d+(\.\d+)?$/.test(value))) {
				          errorMessage = "不能为空且只能为正数";
			             }else{
			               isError = false;
			              }break;
	case "productNumber": if((value == "")|| !(/^[0-9]*[1-9][0-9]*$/.test(value))) {
	...
	//添加新闻验证
	$("#newsAdd").find("[class='text']").bind({focus : function() {focusItem(this)},
	 blur : function() {checkItem(this)}
	});
    ...
    $("#password").bind({focus : function() {focusItem(this)},
	 blur : function() {checkItem(this)}
	});
	...
	//点击换验证码
	$("#changeCode").click(function() {$("#safeCode").attr("src", "Number.jsp?id=" + Math.random());});
	...
	$("#product .thumb img").mouseover(
			function() {
				...
				$("#product").append('<img id="bigImg" src="../Image/Product/' + name+ '_big.jpg"/>');
	...
	$("#main .box dl dt").click(function() {$(this).nextUntil('dt').toggle();});
	...
	$("#shoppingBag").mouseover(
	 function() {
	  ...
	  $("#header").append($table);
	   var html = [ "<tr><th>图标</th><th>名称</th><th>价格</th></tr>","<tr><td>图标</td><td>VVVVV</td><td>99</td></tr>" ]
